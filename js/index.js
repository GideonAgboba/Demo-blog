	...
	app.createProducts = function(){
		...
		for(var i = 0; i < productos.length; i++){
			...
			if(productos[i].stock > 0){
				...
				contenido+= '			<h3>'+productos[i].name+' <span class="price">N '+productos[i].price+' NGN</span></h3>'
				contenido+= '			<h3>Stock available: <span class="stock">'+productos[i].stock+'</span></h3>'
				contenido+= '		</span>'
				contenido+= '		<a class="text-white large-12 columns btn submit ladda-button prod-'+productos[i].id+'" data-style="slide-right" onclick="app.addtoCart('+productos[i].id+');">Add to cart</a>'
